...
use wgpu::{Adapter, Color, CommandEncoderDescriptor, CurrentSurfaceTexture, Device, DeviceDescriptor, Features, FragmentState, Instance, Limits, LoadOp, MemoryHints, Operations, PowerPreference, Queue, RenderPassColorAttachment, RenderPassDescriptor, RenderPipeline, RenderPipelineDescriptor, RequestAdapterOptions, ShaderModuleDescriptor, ShaderSource, StoreOp, Surface, SurfaceConfiguration, TextureFormat, TextureViewDescriptor, VertexState};
...
pub async fn create_graphics(window: Rc<Window>, proxy: EventLoopProxy<Graphics>) {
    ...
    let adapter = instance
        ...
        .await
        .unwrap_or_else(|err| panic!("Failed to find an appropriate adapter: {}", err));
...
impl Graphics {
    ...
    pub fn draw(&mut self) {
        let frame = match self.surface.get_current_texture() {
            ...
            err => panic!("Failed to get current frame: {err:?}"), // TODO: Handle states
        };
