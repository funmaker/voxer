...
impl Benchmark {
	...
	pub fn new_frame(&mut self) {
		...
		self.order.sort_by_key(|name| current.iter()
		                                     .find(|(n, _)| n == name)
		                                     .map(|(_, i)| (i * 100000.0) as u64));
		
		self.frame_start = Instant::now();
	}
	
	pub fn tick(&mut self, stage: &'static str) {
		self.current.push((stage, self.frame_start.elapsed().as_secs_f64()))
	}
	
	pub fn tick_exact(&mut self, stage: &'static str, time: f64) {
		self.current.push((stage, time))
	}
	...
	pub fn on_gui(&mut self, ui: &mut Ui, target_fps: Option<f32>) {
		use egui_plot::*;
		use egui::*;
